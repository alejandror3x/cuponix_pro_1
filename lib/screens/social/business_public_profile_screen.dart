...
class _BusinessPublicProfileScreenState extends State<BusinessPublicProfileScreen> {
  late int _tab; // 0=Actividad, 1=Menu, 2=Valoraciones
            ...
            SafeArea(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  GestureDetector(onTap: () => context.go('/configuracion'), child: Image.asset('assets/images/icon-configuracion.png', width: 36, height: 36)),
                ]),
              ),
            ),
            Expanded(
  ...
  Widget _header() => Column(children: [
    GestureDetector(
      ...
      child: Container(
        width: 164,
        height: 164,
        decoration: const BoxDecoration(color: Color(0xFFD4322B), shape: BoxShape.circle, boxShadow: [BoxShadow(color: Color(0x66000000), blurRadius: 18, offset: Offset(0, 6))]),
        alignment: Alignment.center,
        child: const Text("Roger's", style: TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.w800, fontStyle: FontStyle.italic)),
      ),
    ),
    const SizedBox(height: 12),
  ...
  Widget _stat(String value, String label, VoidCallback onTap) => GestureDetector(
    onTap: onTap,
    child: Column(children: [
      Text(value, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w600)),
      Text(label, style: const TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w500)),
    ]),
  );

  Widget _action(String label, VoidCallback onTap) => GestureDetector(
    onTap: onTap,
    child: Container(height: 34, decoration: BoxDecoration(color: AppColors.neonRed, borderRadius: BorderRadius.circular(999), boxShadow: const [BoxShadow(color: Color(0x66FF073A), blurRadius: 8, offset: Offset(0, 3))]), alignment: Alignment.center, child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 0.8))),
  );
  ...
  Widget _tabs() => Container(
    decoration: BoxDecoration(border: Border.symmetric(horizontal: BorderSide(color: Colors.white.withValues(alpha: 0.35)))),
    child: Row(children: [_tabButton(0, 'assets/images/tab-actividad'), _divider(), _tabButton(1, 'assets/images/tab-menu'), _divider(), _tabButton(2, 'assets/images/tab-valor')]),
  );

  Widget _tabButton(int idx, String key) => Expanded(
    child: GestureDetector(
      onTap: () => setState(() => _tab = idx),
      child: SizedBox(height: 78, child: Center(child: Image.asset('$key-${_tab == idx ? 'activado' : 'desactivado'}.png', width: 54, height: 54))),
    ),
  );
  ...
  Widget _couponCard() => TicketCard(
    aspectRatio: 720 / 240,
    child: Padding(
      padding: const EdgeInsets.fromLTRB(22, 18, 24, 18),
      child: Row(children: [
        const CircleAvatar(radius: 34, backgroundColor: Color(0xFFD4322B), child: Text('R', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800))),
        const SizedBox(width: 18),
        const Expanded(child: Row(children: [_MiniStat(value: '20%', label: 'Descuento'), SizedBox(width: 12), _MiniStat(value: r'$30,00', label: 'Compra mínima'), SizedBox(width: 12), _MiniPoints(points: '5')])),
        _smallPill('Guardar', () => _local('Cupón guardado')),
        const SizedBox(width: 8),
        _smallPill('Usar', () => context.go('/usar-cupon')),
      ]),
    ),
  );

  Widget _textCouponCard() => TicketCard(aspectRatio: 720 / 220, child: Padding(padding: const EdgeInsets.all(22), child: Row(children: const [Expanded(child: Text('Combo smash + papas con bebida gratis los jueves.', style: TextStyle(color: Colors.white, fontSize: 17, height: 1.2, fontWeight: FontWeight.w500))), _MiniPoints(points: '10')])));

  Widget _menuGrid() => GridView.count(
    crossAxisCount: 2,
    shrinkWrap: true,
    ...
    mainAxisSpacing: 12,
    crossAxisSpacing: 12,
    children: const [_ProductTile('Smash Bacon', '25'), _ProductTile('Cheese Burger', '18'), _ProductTile('Papas Roger', '12'), _ProductTile('Malteada', '15')],
  );

  Widget _ratingsPanel() => Column(children: [
    const Text('★★★★☆', style: TextStyle(color: AppColors.neonRed, fontSize: 38, letterSpacing: 3)),
    const SizedBox(height: 8),
    Container(height: 92, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.34), borderRadius: BorderRadius.circular(20)), alignment: Alignment.topLeft, child: const Text('Escribe tu comentario...', style: TextStyle(color: Colors.white70, fontSize: 15))),
    const SizedBox(height: 12),
    GestureDetector(onTap: () => _local('Valoración enviada'), child: Container(height: 34, padding: const EdgeInsets.symmetric(horizontal: 28), decoration: BoxDecoration(color: AppColors.neonRed, borderRadius: BorderRadius.circular(999)), alignment: Alignment.center, child: const Text('CALIFICAR', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, letterSpacing: 1)))),
    const SizedBox(height: 16),
    _review('Alexis', 'Las hamburguesas son muy buenas.'),
    const SizedBox(height: 12),
    _review('Arlyne', 'Buen servicio y puntos faciles de usar.'),
  ]);
...
class _ProductTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      Expanded(child: Container(decoration: BoxDecoration(color: const Color(0xFF6B6B6B), borderRadius: BorderRadius.circular(12)), alignment: Alignment.center, child: const Icon(Icons.fastfood, color: Colors.white70, size: 42))),
      const SizedBox(height: 10),
      ...
      const SizedBox(height: 8),
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [_smallAction(context, 'VER', '/producto-detalle'), const SizedBox(width: 8), _smallAction(context, 'CANJEAR', '/canjear-puntos')]),
    ]),
  );
...
class _MiniStat extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w600)), Text(label, style: const TextStyle(color: Colors.white70, fontSize: 10))]);
}

class _MiniPoints extends StatelessWidget {
  final String points;
  const _MiniPoints({required this.points});
